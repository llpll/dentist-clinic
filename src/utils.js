...
const getRandomDentist = (dentists) => {
  const index = Math.floor(Math.random() * dentists.length)
  return dentists[index]
}

const getRandomAssistant = (assistants) => {
  const index = Math.floor(Math.random() * assistants.length)
...
const getRandomAppointmentIds = (dentists, assistants) => {
  const dentist = getRandomDentist(dentists)
  const assistant = getRandomAssistant(assistants)
  const time = getRandomTime()
  const day = getRandomDay()
  const id = day + ';' + time + ';' + dentist.id + ';' + assistant.id + ''
...
const sortAssortmentsInDay = (a, b) => {
  if (a.time < b.time) {
    return -1
  }
  if (a.time > b.time) {
...
const generateRandomAppointmentsIds = (num, dentists, assitants) =>
  Array(num)
    .fill(0)
    .map((_) => getRandomAppointmentIds(dentists, assitants))
